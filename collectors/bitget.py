...
class BitgetCollector(BaseCollector):
    ...
    async def fetch_tickers(self) -> List[TickerData]:
        ...
        if not data or data.get("code") != "00000":
            logger.error(f"Bitget tickers error: {data}")
            return []
        ...
        for item in data.get("data", []):
            ...
            try:
                ticker = TickerData(
                    ...
                    last_price=float(item.get("lastPr", 0)) or None,
                    bid_price=float(item.get("bidPr", 0)) or None,
                    ask_price=float(item.get("askPr", 0)) or None,
                    volume_24h=float(item.get("quoteVolume", 0)) or None,
                    open_interest=float(item.get("openInterestUsd", 0)) or None,
                ...
                logger.debug(f"Bitget parse error {symbol}: {e}")
        return tickers
    ...
    async def fetch_funding_rates(self) -> Dict[str, Dict]:
        ...
        for item in data.get("data", []):
            ...
            try:
                ...
                logger.debug(f"Bitget funding error {symbol}: {e}")
        return result
    ...
    async def fetch_open_interest(self) -> Dict[str, float]:
        ...
        for item in data.get("data", []):
            ...
            if not symbol.endswith("USDT"):
                continue
            try:
                oi = float(item.get("openInterestUsd", 0))
                if oi > 0:
                    result[symbol] = oi
            except:
                pass
